...
const MEMBERS_QUERY = graphql`
  query Members_MembersQuery($apiKey: String!) {
    orbit(auths: { orbit: { apiKey: $apiKey } }) {
      makeRestCall {
        get(path: "/api/v1/464/members") {
          jsonBody
        }
      }
    }
...
export function MembersQuery(props) {
  const data = useLazyLoadQuery(MEMBERS_QUERY, props, {
...
export default function MembersQueryForm(props) {
  ...
  return (
    <div>
      ...
      >
        <Suspense fallback={"Loading MembersQuery..."}>
          <MembersQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
