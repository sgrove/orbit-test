...
const WORKSPACES_QUERY = graphql`
  query RestWorkspaces_restWorkspacesQuery($apiKey: String!) {
...
export function RestWorkspacesQuery(props) {
  ...
  const dataEl = data ? (
    <div className="data-box">
      <h3>
        Data for RestWorkspaces <LocationNote />
      </h3>
...
export default function RestWorkspacesQueryForm(props) {
  ...
  const actionButtonEl = hasError ? null : (
    <button onClick={() => setQueryVariables({ ...formVariables })}>
      Run RestWorkspacesQuery
    </button>
  );

  return (
    <div>
      <h3>RestWorkspaces</h3>
      {formEl}
      {actionButtonEl}
      <ErrorBoundary
      ...
      >
        <Suspense fallback={"Loading RestWorkspacesQuery..."}>
          <RestWorkspacesQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
