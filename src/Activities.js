...
const ACTIVITIES_QUERY = graphql`
  query Activities_ActivitiesQuery($apiKey: String!) {
    orbit(auths: { orbit: { apiKey: $apiKey } }) {
      makeRestCall {
        get(path: "/api/v1/464/activities") {
          jsonBody
        }
      }
    }
...
export function ActivitiesQuery(props) {
  const data = useLazyLoadQuery(ACTIVITIES_QUERY, props, {
  ...
  const dataEl = data ? (
    <div className="data-box">
      <h3>
        Data for Activities <LocationNote />
      </h3>
...
export default function ActivitiesQueryForm(props) {
  ...
  return (
    <div>
      ...
      >
        <Suspense fallback={"Loading ActivitiesQuery..."}>
          <ActivitiesQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
