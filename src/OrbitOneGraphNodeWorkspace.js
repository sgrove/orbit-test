...
const ORBIT_ONE_GRAPH_NODE_WORKSPACE_QUERY = graphql`
  query OrbitOneGraphNodeWorkspace_OrbitOneGraphNodeWorkspaceQuery(
    $apiKey: String!
  ) {
    oneGraphNode(
      auths: { orbit: { apiKey: $apiKey } }
      oneGraphId: "MTpvcmJpdDpNVHBQY21KcGRGZHZjbXR6Y0dGalpUbzBOalE"
    ) {
      ... on OrbitWorkspace {
        name
        id
...
export function OrbitOneGraphNodeWorkspaceQuery(props) {
  const data = useLazyLoadQuery(ORBIT_ONE_GRAPH_NODE_WORKSPACE_QUERY, props, {
  ...
  const dataEl = data ? (
    <div className="data-box">
      <h3>
        Data for OrbitOneGraphNodeWorkspace <LocationNote />
      </h3>
...
export default function OrbitOneGraphNodeWorkspaceQueryForm(props) {
  ...
  const actionButtonEl = hasError ? null : (
    <button onClick={() => setQueryVariables({ ...formVariables })}>
      Run OrbitOneGraphNodeWorkspaceQuery
    </button>
  );

  return (
    <div>
      <h3>OrbitOneGraphNodeWorkspace</h3>
      {formEl}
      {actionButtonEl}
      <ErrorBoundary
      ...
      >
        <Suspense fallback={"Loading OrbitOneGraphNodeWorkspaceQuery..."}>
          <OrbitOneGraphNodeWorkspaceQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
