...
const WORKSPACE_QUERY = graphql`
  query Workspace_WorkspaceQuery($apiKey: String!) {
    orbit(auths: { orbit: { apiKey: $apiKey } }) {
      makeRestCall {
        get(path: "/api/v1/workspaces/464") {
          jsonBody
        }
      }
    }
...
export function WorkspaceQuery(props) {
  const data = useLazyLoadQuery(WORKSPACE_QUERY, props, {
  ...
  const dataEl = data ? (
    <div className="data-box">
      <h3>
        Data for Workspace <LocationNote />
      </h3>
...
export default function WorkspaceQueryForm(props) {
  ...
  return (
    <div>
      ...
      >
        <Suspense fallback={"Loading WorkspaceQuery..."}>
          <WorkspaceQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
